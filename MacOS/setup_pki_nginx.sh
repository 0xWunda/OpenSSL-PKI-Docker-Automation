...
##############################################
#
#  Auto-PKI + Docker/Nginx Setup Script (macOS)
#  Made by 0xWunda
...
if command -v /opt/homebrew/opt/openssl@3/bin/openssl >/dev/null 2>&1; then
  ...
  echo "Fehler: OpenSSL wurde nicht gefunden. Bitte installiere openssl@3 über Homebrew."
  exit 1
fi
...
if [[ ! -f "$PKI_DIR/root/certs/ca.cert.pem" ]]; then
  ...
  echo " -> Root-Zertifikat existiert bereits, überspringe."
fi
...
if [[ ! -f "$PKI_DIR/intermediate/private/intermediate.key.pem" ]]; then
  echo " -> Generiere Intermediate-Key"
...
echo " -> Erstelle Intermediate-CSR"
...
if [[ ! -f "$PKI_DIR/intermediate/certs/intermediate.cert.pem" ]]; then
  echo " -> Signiere Intermediate-CSR mit Root-CA"
  ...
  echo " -> Intermediate-Zertifikat existiert bereits, überspringe Signatur."
fi

echo " -> CA-Chain erzeugen"
...
echo " -> Erste CRL für Intermediate erzeugen"
...
echo
echo "OCSP-Responder kannst du manuell z.B. so starten (in anderem Terminal):"
...
echo
echo "Zum Revoken eines Zertifikats + CRL-Update:"
